    ...
    $.ajax({
        ...
        success: function(response){ 
         for (var i = 0; i < response.length; i++) {
            var html = '<a href="/milestone/'+response[i]['id_milestone']+'" class="dropdown-item">'
                        +'<div class="media">'
                        +'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-clock"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>'
                        +'<div class="media-body">'
                        +'<div class="notification-para"><span class="user-name">Pengingat : </span> Waktu produksi untuk Order #'+response[i]['id_order']+' akan berakhir '+response[i]['diff']+' hari lagi ('+response[i]['end_date']+')</div>'
                        +'</div>'
                        +'</div>'
                        +'</a>';
            $('#notif').append(html);
